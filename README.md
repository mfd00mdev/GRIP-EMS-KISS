# K.I.S.S. (Keep It Simple, Stupid)

<p align="center">
  <img src="Docs/Screenshots/kiss-main.png" alt="K.I.S.S. window">
</p>

GRIP-EMS-KISS is a plugin for [GRIP-EMS](https://www.curseforge.com/wow/addons/grip-enhanced-macro-sequencer) (GRIP - Enhanced Macro Sequencer),that walks you through the three things you need to do to get it working: select a sequence, bind a key to it, then test it out. The acronym stands for Keep It Simple, Stupid. A newcomer to the addon may feel overwhelmed by all the settings and options. This addon aims to make it as simple as possible to get you rockin' and rollin' with the GRIP-EMS addon.

![K.I.S.S. version](https://img.shields.io/badge/version-0.1.2-blue)

## Features

- A guided three step wizard anyone can follow.
- Select from sequences you already have, or loads GRIP-EMS's import window if you need to load one. Includes links to external sites for finding rotations.
- Lets you set a keybind with one click, applied through GRIP-EMS automatically.
- A final page with tips if something isn't working, plus a link to the [GRIP-EMS Discord](https://discord.gg/XQXH3nt2X8) 

## Requirements

- World of Warcraft: Retail 12.1+
- [GRIP-EMS](https://www.curseforge.com/wow/addons/grip-enhanced-macro-sequencer) installed and enabled. K.I.S.S. is just a companion plugin and won't load without it

## How to install it

- Download or clone this repository.
- Copy the `GRIP-EMS-KISS` folder into your `World of Warcraft/_retail_/Interface/AddOns/` folder.
- Start WoW and enable **GRIP - K.I.S.S. - Keep It Simple, Stupid** in your AddOns list.

   ![K.I.S.S. in Addon List](Docs/Screenshots/kiss-list.png)

## Options and commands

- The first time you log in, the guided wizard opens on its own.
- You can reopen it anytime by typing `/gems kiss`

## Credits

Made by MFDOOM with love.

## License

See [LICENSE](LICENSE).
